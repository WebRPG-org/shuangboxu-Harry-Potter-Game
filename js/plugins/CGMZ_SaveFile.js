	...
	for(const infoType of CGMZ.SaveFile.DisplayInfo) {
		...
		switch(infoType) {
			case "Image":
				...
				this._neededHeight += CGMZ.SaveFile.ImageHeight;
				break;
			case "Image And Location":
				this.drawSaveBitmap(bitmap);
				this._neededHeight += CGMZ.SaveFile.ImageHeight - this.lineHeight();
...
// ---------- CGMZ SaveFile: 图片预览补丁（追加到插件文件中） ----------

// 初始化预览 Sprite（如果 CGMZ_Window_SaveFileDisplay.prototype.initialize 已存在，
// 可把下面这段合并到原 initialize 中）
if (!CGMZ_Window_SaveFileDisplay.prototype.CGMZ__initPreview) {
    CGMZ_Window_SaveFileDisplay.prototype.CGMZ__initPreview = function() {
        // 如果已经存在则跳过
        if (this._cgmzPreviewSprite) return;
        this._cgmzPreviewSprite = new Sprite();
        // anchor 用来居中显示
        this._cgmzPreviewSprite.anchor = new Point(0.5, 0.5);
        // 初始不可见
        this._cgmzPreviewSprite.visible = false;
        this.addChild(this._cgmzPreviewSprite);
        this._cgmzCurrentImage = null;
    };
}

// 设置显示 info 时调用（如果已有 setInfo，请把这段逻辑合并进去）
if (!CGMZ_Window_SaveFileDisplay.prototype.setInfoWithPreview) {
    const _orig_setInfo = CGMZ_Window_SaveFileDisplay.prototype.setInfo || function() {};
    CGMZ_Window_SaveFileDisplay.prototype.setInfo = function(info) {
        // 先运行原本行为（如果有）
        try { _orig_setInfo.apply(this, arguments); } catch(e) { /* 忽略 */ }

        this.CGMZ__initPreview();

        const url = info && info.cgmz_image ? info.cgmz_image : "";
        // 如果没有图片，隐藏预览
        if (!url) {
            this._cgmzCurrentImage = null;
            if (this._cgmzPreviewSprite) {
                this._cgmzPreviewSprite.visible = false;
                this._cgmzPreviewSprite.bitmap = null;
            }
            return;
        }

        // 如果同一张图片已经在显示，则不重复加载
        if (this._cgmzCurrentImage === url) return;
        this._cgmzCurrentImage = url;

        // 处理路径：通常把图片放在 img/pictures/ 下，然后在地图备注写文件名（例如 "mypic.png"）
        // 如果你在备注里写 "img/pictures/mypic.png" 或者 "pictures/mypic.png" 也尽量支持。
        // 尝试用 ImageManager 加载（MZ 的 ImageManager 能接受相对 img/ 下的路径）
        let path = url;
        // 如果用户只写了文件名（无斜杠），把它当作 pictures 下的文件
        if (!path.includes("/")) {
            // ImageManager.loadBitmap 的第一个参数是目录（相对 img/），第二个是文件名（不含扩展名也可）
            const filename = path.replace(/^\/+|img\/|pictures\//g, "");
            const bmp = ImageManager.loadBitmap("img/pictures/", filename);
            bmp.addLoadListener(() => { this._cgmz_setPreviewBitmap(bmp); });
        } else {
            // 用户写了包含路径的字符串（可能为 "img/pictures/xxx.png" 或 "pictures/xxx.png"）
            // 去掉前面的 "img/"，交给 ImageManager 加载相对路径
            let normalized = path.replace(/^img\//, "");
            // 交给 ImageManager.loadBitmap('', normalized) 让它按相对 img/ 加载
            const bmp = ImageManager.loadBitmap("", normalized);
            bmp.addLoadListener(() => { this._cgmz_setPreviewBitmap(bmp); });
        }
    };

    // 把 bitmap 显示到 sprite，并按窗口尺寸计算缩放与居中
    CGMZ_Window_SaveFileDisplay.prototype._cgmz_setPreviewBitmap = function(bitmap) {
        if (!this._cgmzPreviewSprite) this.CGMZ__initPreview();
        // 取消平滑以保留像素风（需要像素化时保留）：
        try { bitmap.smooth = false; } catch(e) { /* 有些环境可能不支持 */ }

        this._cgmzPreviewSprite.bitmap = bitmap;
        this._cgmzPreviewSprite.visible = true;

        // 等待 bitmap 加载完成（宽高 > 0）
        if (!bitmap.width || !bitmap.height) {
            // addLoadListener 已确保加载完成才调用，但保险起见再判断一次
            bitmap.addLoadListener(() => { this._cgmz_layoutPreview(); });
        } else {
            this._cgmz_layoutPreview();
        }
    };

    // 根据窗口大小计算缩放（保持长宽比）并居中
    CGMZ_Window_SaveFileDisplay.prototype._cgmz_layoutPreview = function() {
        if (!this._cgmzPreviewSprite || !this._cgmzPreviewSprite.bitmap) return;

        const bmp = this._cgmzPreviewSprite.bitmap;
        const pad = this.padding || 8;
        const areaW = this.width - pad * 2;
        const areaH = this.height - pad * 2;

        // 防止 0 除
        if (bmp.width === 0 || bmp.height === 0) return;

        let scale = 1.0;
        if (CGMZ && CGMZ.SaveFile && CGMZ.SaveFile.StretchImage) {
            // 如果插件设置允许拉伸，则填满（可能改变长宽比）
            const sx = areaW / bmp.width;
            const sy = areaH / bmp.height;
            this._cgmzPreviewSprite.scale.x = sx;
            this._cgmzPreviewSprite.scale.y = sy;
        } else {
            // 保持长宽比，等比缩放至能完整放入区域
            scale = Math.min(areaW / bmp.width, areaH / bmp.height, 1.0);
            this._cgmzPreviewSprite.scale.x = scale;
            this._cgmzPreviewSprite.scale.y = scale;
        }

        // 居中位置（使用 anchor = 0.5）
        this._cgmzPreviewSprite.x = pad + areaW / 2;
        this._cgmzPreviewSprite.y = pad + areaH / 2;
    };
}
